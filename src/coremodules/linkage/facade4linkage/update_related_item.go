...
func updateRelatedItem(
	...
	relateds []*dbo4linkage2.RelationshipItemRolesCommand,
) (recordsUpdates []record.Updates, err error) {
	itemKey := dbo4spaceus.NewSpaceModuleItemKeyFromItemRef(objectRef)
	itemDbo := new(dbo4linkage2.WithRelatedAndIDsAndUserID)
	itemDbo.WithRelatedAndIDs = new(dbo4linkage2.WithRelatedAndIDs)
