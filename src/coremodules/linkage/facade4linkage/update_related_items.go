...
func UpdateRelatedItemsWithLatestRelationships(
	...
	itemData dbo4linkage2.WithRelatedAndIDs,
) (err error) {
...
func updateItemWithLatestRelationshipsFromRelatedItem(
	...
	relatedItemRef dbo4linkage2.SpaceModuleItemRef,
	relatedByModuleOfRelatedItem dbo4linkage2.RelatedByModuleID,
) (err error) {
	itemRelationshipInRelatedItem := dbo4linkage2.GetRelatedItemByRef(relatedByModuleOfRelatedItem, itemRef, false)
	...
	return db.RunReadwriteTransaction(ctx, func(ctx context.Context, tx dal.ReadwriteTransaction) (err error) {
		...
		item := record.NewDataWithID(itemRef.ItemID, key, new(dbo4linkage2.WithRelatedAndIDsAndUserID))
		if err = tx.Get(ctx, item.Record); err != nil {
			return err
		}
		relatedItem := dbo4linkage2.GetRelatedItemByRef(item.Data.Related, relatedItemRef, true)
