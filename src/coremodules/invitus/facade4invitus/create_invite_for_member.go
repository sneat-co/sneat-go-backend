...
import (
	...
	dbo4invitus2 "github.com/sneat-co/sneat-go-backend/src/coremodules/invitus/dbo4invitus"
...
type InviteMemberRequest struct {
	...
	To    dbo4invitus2.InviteToMember `json:"to"`
...
type CreateInviteResponse struct {
	Invite dbo4invitus2.InviteBrief `json:"invite"`
}
...
func CreateOrReuseInviteForMember(ctx context.Context, userCtx facade.UserContext, request InviteMemberRequest) (response CreateInviteResponse, err error) {
	...
	err = dal4contactus.RunContactusSpaceWorker(ctx, userCtx, request.SpaceRequest,
		func(ctx context.Context, tx dal.ReadwriteTransaction, params *dal4contactus.ContactusSpaceWorkerParams) (err error) {
			...
			var (
				inviteID       string
				personalInvite *dbo4invitus2.PersonalInviteDbo
			)
			...
			if memberInviteBrief != nil {
				...
				if personalInvite.Status == "active" {
					...
					personalInvite = nil
					inviteBriefs := make([]*dbo4invitus2.MemberInviteBrief, 0, len(fromContact.Data.Invites)-1)
			...
			response.Invite = dbo4invitus2.NewInviteBriefFromDto(inviteID, personalInvite.InviteDbo)
			if !request.Send {
...
func createPersonalInvite(
	...
	inviteID string, personalInvite *dbo4invitus2.PersonalInviteDbo, err error,
) {
	...
	from := dbo4invitus2.InviteFrom{
		InviteContact: dbo4invitus2.InviteContact{
			UserID:   uid,
	...
	inviteSpace := dbo4invitus2.InviteSpace{
		ID:    request.SpaceID,
	...
	fromMember.Data.Invites = append(fromMember.Data.Invites, &dbo4invitus2.MemberInviteBrief{
		ID:         inviteID,
