...
func delayedUpdateBillCards(c context.Context, billID string, footer string) error {
	...
	if bill, err := facade4splitus.GetBillByID(c, nil, billID); err != nil {
		...
		for _, tgChatMessageID := range bill.Data.TgChatMessageIDs {
			if err = delayUpdateBillTgChatCard.EnqueueWork(c, delaying.With(const4splitus.QueueSplitus, "update-bill-tg-chat-card", 0), billID, tgChatMessageID, footer); err != nil {
