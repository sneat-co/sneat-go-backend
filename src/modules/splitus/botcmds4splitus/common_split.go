...
func getSplitParamsAndCurrentMember(q url.Values, members []*briefs4splitus.BillMemberBrief) (member *briefs4splitus.BillMemberBrief, add int, err error) {
	if len(members) == 0 {
	...
	if memberID := q.Get("m"); memberID == "" {
		...
		var (
			i    int
			m    *briefs4splitus.BillMemberBrief
			move string
		)
...
func writeSplitMembers(buffer *bytes.Buffer, members []*briefs4splitus.BillMemberBrief, currentMemberID string, currency money.CurrencyCode) {
	var totalShares int
