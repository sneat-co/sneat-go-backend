...
func (ti TransferInterest) Validate() error {
	if !interest.IsKnownFormula(ti.InterestType) {
		return validation.NewErrBadRecordFieldValue("interestType", fmt.Sprintf("unknown interest type: %v", ti.InterestType))
	}
	if ti.InterestPercent < 0 {
		return validation.NewErrBadRecordFieldValue("interestPercent", fmt.Sprintf("interest percent < 0: %v", ti.InterestPercent))
	}
	if ti.InterestGracePeriod < 0 {
		return validation.NewErrBadRecordFieldValue("interestGracePeriod", fmt.Sprintf("interest grace period < 0: %v", ti.InterestGracePeriod))
	}
	if ti.InterestMinimumPeriod < 0 {
		return validation.NewErrBadRecordFieldValue("interestMinimumPeriod", fmt.Sprintf("interest minimum period < 0: %v", ti.InterestMinimumPeriod))
	}
	if ti.InterestPeriod < 0 {
		return validation.NewErrBadRecordFieldValue("interestPeriod", fmt.Sprintf("interest period < 0: %v", ti.InterestPeriod))
	}
	return nil
}
