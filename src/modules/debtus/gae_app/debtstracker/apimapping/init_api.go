...
func InitApi(router *httprouter.Router) {
	...
	POST("/api4debtus/auth/sign-in-with-pin", api4debtus.OptionalAuth(unsorted.HandleSignInWithPin))
	POST("/api4debtus/auth/sign-in-with-fbm", api4debtus.OptionalAuth(unsorted.HandleSignInWithFbm))
	POST("/api4debtus/auth/sign-in", api4debtus.OptionalAuth(unsorted.HandleSignInWithCode))
	POST("/api4debtus/auth/fb/signed", api4debtus.OptionalAuth(unsorted.HandleSignedWithFacebook))
	POST("/api4debtus/auth/google-plus/signed", api4debtus.OptionalAuth(unsorted.HandleSignedInWithGooglePlus))
	POST("/api4debtus/auth/vk/signed", api4debtus.OptionalAuth(unsorted.HandleSignedWithVK))
	POST("/api4debtus/auth/email-sign-up", unsorted.HandleSignUpWithEmail)
	POST("/api4debtus/auth/email-sign-in", unsorted.HandleSignInWithEmail)
	POST("/api4debtus/auth/request-password-reset", unsorted.HandleRequestPasswordReset)
	POST("/api4debtus/auth/change-password-and-sign-in", unsorted.HandleChangePasswordAndSignIn)
	POST("/api4debtus/auth/confirm-email-and-sign-in", unsorted.HandleConfirmEmailAndSignIn)
	POST("/api4debtus/auth/anonymous-sign-up", unsorted.HandleSignUpAnonymously)
	POST("/api4debtus/auth/anonymous-sign-in", unsorted.HandleSignInAnonymous)
	POST("/api4debtus/auth/disconnect", api4debtus.AuthOnly(unsorted.HandleDisconnect))
