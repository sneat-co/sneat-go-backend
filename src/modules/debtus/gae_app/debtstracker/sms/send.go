...
func SendSms(c context.Context, isLive bool, toPhoneNumber, smsText string) (isTestSender bool, smsResponse *gotwilio.SmsResponse, twilioException *gotwilio.Exception, err error) {
	...
	if isLive {
		accountSid = common4debtus.TWILIO_LIVE_ACCOUNT_SID
		accountToken = common4debtus.TWILIO_LIVE_ACCOUNT_TOKEN
		fromNumber = common4debtus.TWILIO_LIVE_FROM_US
	} else {
		accountSid = common4debtus.TWILIO_TEST_ACCOUNT_SID
		accountToken = common4debtus.TWILIO_TEST_ACCOUNT_TOKEN
	...
	if smsResponse, twilioException, err = twilio.SendSMS(
		...
		common4debtus.TWILIO_APPLICATION_SID,
	); err != nil {
		return
	}
