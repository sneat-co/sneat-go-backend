...
const counterpartyButtonsLimit = 4
	...
	if webhookMessage, ok := whc.Input().(botsfw.WebhookTextMessage); ok && webhookMessage.Text() == showAllContactsText {
		...
		case 0: // Space has no active debts
			if contactusSpaceDbo.TotalContactsCountByStatus[briefs4contactus.ContactStatusActive] > 0 {
				...
				counterparties2buttons(contacts, false)
			} else {
		...
		default: // UserEntry have active debts (balance is not 0).
			...
			if len(contactsToShow) <= counterpartyButtonsLimit {
				...
				for _, latestCounterparty := range latestCounterparties {
					...
					for contactToShowID := range contactsToShow {
						if contactToShowID == latestCounterparty.ContactID {
							isInWithDebts = true
							break
						}
					}
					if !isInWithDebts {
						contactBrief := contactusSpaceDbo.Contacts[latestCounterparty.ContactID]
						contactsToShow[latestCounterparty.ContactID] = contactBrief
					}
					if len(contactsToShow) >= counterpartyButtonsLimit {
						break
					}
			...
			counterparties2buttons(contactsToShow, false)
		}
	}
	if len(buttons) > 0 {
...
func NewTransferWizard(whc botsfw.WebhookContext) (TransferWizard, error) {
	...
	logus.Debugf(whc.Context(), "AwaitingReplyTo: %v", awaitingReplyTo)
	...
	telegramKeyboard.InlineKeyboard = append(
		...
		[]tgbotapi.InlineKeyboardButton{
			tgbotapi.NewInlineKeyboardButtonData(
				...
				SendReceiptCallbackData(transfer.ID, ReceiptActionDoNotSend), // TODO: Replace path with constant
			),
		},
	)
