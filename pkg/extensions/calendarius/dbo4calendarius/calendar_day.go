...
// CalendarDayDbo is a record in "spaces/{spaceID}/calendarius/days" collection with ContactID=YYYY-MM-DD
...
// NewCalendarDayKey returns key for a record in spaces/{spaceID}/calendarius/days collection with ContactID=YYYY-MM-DD
func NewCalendarDayKey(spaceID coretypes.SpaceID, date string) *dal.Key {
	return dbo4spaceus.NewSpaceModuleItemKey(spaceID, const4calendarius.ExtensionID, DaysCollection, date)
}
