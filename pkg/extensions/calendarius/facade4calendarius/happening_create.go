...
func CreateHappening(
	ctx facade.ContextWithUser, request dto4calendarius.CreateHappeningRequest,
) (
	response dto4calendarius.CreateHappeningResponse, err error,
) {
	request.Happening.Title = strings.TrimSpace(request.Happening.Title)
	if err = request.Validate(); err != nil {
		return
	}
	happeningDto := &dbo4calendarius.HappeningDbo{
	...
	if happeningDto.Type == dbo4calendarius.HappeningTypeSingle {
	...
	err = dal4spaceus2.CreateSpaceItem(ctx, request.SpaceRequest,
		...
		func(ctx facade.ContextWithUser, tx dal.ReadwriteTransaction, params *dal4spaceus2.ModuleSpaceWorkerParams[*dbo4calendarius.CalendariusSpaceDbo]) (err error) {
...
func createHappeningTx(
	...
	happeningDbo *dbo4calendarius.HappeningDbo,
	params *dal4spaceus2.ModuleSpaceWorkerParams[*dbo4calendarius.CalendariusSpaceDbo],
) (
	response dto4calendarius.CreateHappeningResponse, err error,
) {
	...
	if happeningDbo.Type == dbo4calendarius.HappeningTypeSingle {
	...
	if happeningDbo.Type == dbo4calendarius.HappeningTypeRecurring {
		...
		calendarHappeningBrief := &dbo4calendarius.CalendarHappeningBrief{
		...
		params.SpaceModuleUpdates = append(params.SpaceUpdates,
			update.ByFieldPath([]string{dbo4calendarius.RecurringHappeningsField, happeningID}, calendarHappeningBrief))
	}
	return
}
