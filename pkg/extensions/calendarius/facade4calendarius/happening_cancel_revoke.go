...
func RevokeHappeningCancellation(ctx facade.ContextWithUser, request dto4calendarius.CancelHappeningRequest) (err error) {
...
func removeCancellationFromHappeningBriefInSpaceModuleEntry(params *dal4calendarius.HappeningWorkerParams) (err error) {
	...
	if updates := happeningBrief.RemoveCancellation(); len(updates) > 0 {
		for _, u := range updates {
			params.SpaceModuleUpdates = append(params.SpaceModuleUpdates,
				update.ByFieldPath(
					[]string{dbo4calendarius.RecurringHappeningsField, params.Happening.ID, u.FieldName()},
					u.Value()),
			)
		}
...
func removeCancellationFromCalendarDay(ctx context.Context, tx dal.ReadwriteTransaction, happeningID string, slotID string, calendarDay dbo4calendarius.CalendarDayEntry) error {
