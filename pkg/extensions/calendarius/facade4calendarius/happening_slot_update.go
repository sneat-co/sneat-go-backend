...
func UpdateHappeningSlot(ctx facade.ContextWithUser, putMode PutMode, request dto4calendarius.HappeningSlotRequest) (err error) {
	if err = request.Validate(); err != nil {
		return validation.NewBadRequestError(err)
	}

	worker := func(ctx facade.ContextWithUser, tx dal.ReadwriteTransaction, params *dal4calendarius.HappeningWorkerParams) (err error) {
		return updateHappeningSlotTxWorker(ctx, tx, params, putMode, request)
	}

	return dal4calendarius.RunHappeningSpaceWorker(ctx, request.HappeningRequest, worker)
}

func updateHappeningSlotTxWorker(ctx context.Context, tx dal.ReadwriteTransaction, params *dal4calendarius.HappeningWorkerParams, putMode PutMode, request dto4calendarius.HappeningSlotRequest) (err error) {
	...
	params.HappeningUpdates = []update.Update{
		update.ByFieldPath([]string{dbo4calendarius.SlotsField, request.Slot.ID}, slot),
	}
	if params.Happening.Data.Type == dbo4calendarius.HappeningTypeSingle {
	...
	if params.Happening.Data.Type == dbo4calendarius.HappeningTypeRecurring {
		if happeningBrief := params.SpaceModuleEntry.Data.GetRecurringHappeningBrief(params.Happening.ID); happeningBrief != nil {
			...
			params.SpaceModuleUpdates = append(params.SpaceModuleUpdates, update.ByFieldPath(
				[]string{dbo4calendarius.RecurringHappeningsField, params.Happening.ID, dbo4calendarius.SlotsField, request.Slot.ID},
				slot,
			))
		}
	}
