...
while true; do
    ...
    case $app in
        dev )
        	sed -i '' 's/^application: *[[:alpha:]]*-[[:alnum:]]*/application: debtstracker-dev1/' app.yaml
        	break;;
        prod )
        	sed -i '' 's/^application: *[[:alpha:]]*-[[:alnum:]]*/application: debtstracker-io/' app.yaml
        	break;;
...
if [ $? -ne 0 ]; then
    echo "Tests failed"
	sed -i '' 's/^application: *[[:alpha:]]*-[[:alnum:]]*/application: debtstracker-local/' app.yaml
	exit 1
fi
